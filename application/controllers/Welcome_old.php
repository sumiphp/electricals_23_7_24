...
public function deletepack(){
	...
	$this->db->where('packageid',$id);
	$this->db->delete('packagesdetails');
...
public function deleteenquiries(){
	...
	$this->db->delete('packageenquiries');
...
public function addflightdestprocess(){
	...
	$data = array(
	 ...
	 //print_r($data);
	 $this->db->insert('touristplaces', $data);
...
public function adddurationprocess(){
	...
	$data = array(
	 ...
	 //print_r($data);
	 $this->db->insert('tourduration', $data);

	 echo ($this->db->affected_rows() != 1) ? 'Error in Adding duration' : '<b>Duration added Successfully</b>';
}
...
public function editflightprocess(){
	...
	$data = array(
	 ...
	 $this->db->update('touristplaces', $data);

	 echo ($this->db->affected_rows() != 1) ? 'Error in Editing touristplaces' : '<b>Touristplaces edited Successfully</b>';
}
...
function delduration(){
	...
	$id=$_GET['id'];
	$this->db->where('durationid',$id);
	$this->db->delete('tourduration');
	echo ($this->db->affected_rows() != 1) ? 'Error in deleting duration' : 'Duration deleted Successfully';
...
public function editpckprocess(){
	...
	$id=$this->input->post('id');
	$this->db->where('pckid',$id);
	$this->db->select('*');
	$this->db->from('packagepage');
	$query = $this->db->get();
   $imgdetails=$query->row();
   $image11=$imgdetails->picture;
   $image22=$imgdetails->picture;
   //$file_ext = pathinfo($_FILES["image1"]["name"],PATHINFO_EXTENSION);
	...
	//}
	  ...
	  //die;
	  $this->db->where('pckid',$id);
	   $this->db->update('packagepage', $data);
	   //$this->db->insert('problems', $data);

	  //echo ($this->db->affected_rows() != 1) ? 'Error in Adding Solutions' : '<b>Solutions Added Successfully</b>';
	  ($this->db->affected_rows() != 1) ? $this->session->set_flashdata('flash_msg', 'Error in Editing Package Page') : $this->session->set_flashdata('flash_msg', 'Package Page Edited Successfully');
...
public function addauthorityprocess(){
	...
	//$link=$this->input->post('link');
	 ...
	 $data = array(
	  ...
	  echo ($this->db->affected_rows() != 1) ? 'Error in Adding authority' : '<b>Authority Added Successfully</b>';
   ...
   $image11=$imgdetails->picture;
	...
	//$link=$this->input->post('link');
	 ...
	 $data = array(
	  ...
	  $id=$this->input->post('id');
	  $this->db->where('imgid',$id);
	   $this->db->update('packagesgallery', $data);
	   //$this->db->insert('problems', $data);
	   
	   //$this->db->insert('productimages', $data);
	  echo ($this->db->affected_rows() != 1) ? 'Error in Adding Picture' : '<b>Picture Added Successfully</b>';
   ...
   'noofstars '=>"$noofstars",
	...
	'alttagimg1'=>"$alttag1",'active'=>"$status"		
	 ...
	 //$this->db->update('testimonials', $data);
	  ...
	  echo ($this->db->affected_rows() != 1) ? 'Error in Editing Tour Services' : '<b>Tour Services Edited Successfully</b>';
	...
	$title2=$this->input->post('title2');
	 ...
	 $data = array(
	  ...
	  //$this->db->where('testimonialid',$id);
	   ...
	   $this->db->insert('packagesdetails', $data);
	  echo ($this->db->affected_rows() != 1) ? 'Error in Adding Package details' : '<b>Package details Added Successfully</b>';
...
//$this->email->to($list);
  ...
  $this->email->subject($subject); // replace it with relevant subject
     ...
     $body = $this->load->view('winsky/subscriberssemail1.php',$data,TRUE);
	//die;
